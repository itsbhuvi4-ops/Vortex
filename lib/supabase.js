...
async function ensureAdminUserExists() {
  if (!supabase) return;
  const adminEmail = (process.env.ADMIN_EMAIL || 'admin@vortexclash.com').toLowerCase().trim();
...
async function registerTeamInDb(teamData, maxTeams = 30) {
  ...
  // 1. Check duplicate phone / whatsapp
  ...
  // 3. Generate sequential registration ID atomically
  let regId = null;
  try {
