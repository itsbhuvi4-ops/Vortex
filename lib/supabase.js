...
async function removeFromSupabaseStorage(bucket, identifier) {
  if (!identifier) return;

  if (identifier.startsWith('/uploads/')) {
    const localPath = path.join(__dirname, identifier.replace(/^\/uploads\//, ''));
  ...
  if (filename.startsWith(`storage:${bucket}/`)) {
    filename = filename.replace(`storage:${bucket}/`, '');
  } else if (filename.includes(`/storage/v1/object/public/${bucket}/`)) {
    filename = filename.split(`/storage/v1/object/public/${bucket}/`).pop();
  }

  const { error } = await supabase.storage.from(bucket).remove([filename]);
  if (error) {
    console.warn(`[SUPABASE STORAGE] Cleanup failed for ${bucket}:`, error.message);
  }
}
...
async function getSettingsFromDb() {
  if (supabase) {
    ...
    if (error) {
      console.error('[SUPABASE ERROR] getSettingsFromDb:', error.message);
      throw error;
    }
...
async function getTeamCountFromDb() {
  ...
  if (error) {
    console.error('[SUPABASE ERROR] getTeamCountFromDb:', error.message);
    throw error;
  }
  return count || 0;
}
...
async function registerTeamInDb(teamData, maxTeams = 30) {
  ...
  // Registration must go through the database function so team and player rows
  ...
  try {
    ...
    if (rpcErr) {
      console.error('[SUPABASE ERROR] register_team_atomic:', rpcErr);
      throw new Error(`Registration database function failed: ${rpcErr.message || 'unknown RPC error'}`);
    }
    if (!rpcRes) {
      throw new Error('Registration database function returned no result. Apply the current Supabase migration.');
    }
    if (!rpcRes.success) {
      return rpcRes;
    }
    ...
    console.error('[SUPABASE ERROR] registerTeamInDb:', err);
    throw err;
  }
}
...
async function deleteTeamFromDb(idOrRegId) {
  ...
  const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(String(idOrRegId));
  const identifierColumn = isUuid ? 'id' : 'registration_id';
  const { data: team, error: findError } = await supabase
    .from('teams')
    .select('id, payment_proof_url, team_logo_url')
    .eq(identifierColumn, idOrRegId)
    .maybeSingle();

  if (findError) {
    console.error('[SUPABASE ERROR] find team for deletion:', findError.message);
    throw findError;
  }
  if (!team) return null;
